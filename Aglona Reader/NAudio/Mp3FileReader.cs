...
namespace NAudio.Wave
{
    ...
    public class Mp3FileReader : WaveStream
    {
        ...
        private byte startupFrames = 0;

        private long lastFrameToPlay;
        private int lastSampleToPlay;
        ...
        public Mp3FileReader(Stream inputStream, FrameDecompressorBuilder frameDecompressorBuilder)
        {
            ...
            // Fields used to trim playback if required
            lastFrameToPlay = -1;
            lastSampleToPlay = 0;
            firstSkipSamples = 0;
            ...
            this.totalMilliseconds = TotalSeconds() * 1000.0;

            this.samplesPerFrame = firstFrame.SampleCount;
        ...
        /// <summary>
        /// Gets the total length of this file in seconds.
        /// </summary>
        ...
        public override int Read(byte[] sampleBuffer, int offset, int numBytes)
        {
            ...
            if (lastFrameToPlay != -1 && tocIndex > lastFrameToPlay)
                return 0;
            ...
            lock (repositionLock)
            {
                ...
                {
                    // Technical limitation: impossible to have both ends of the frame trimmed.
                    // If the last sample to play is set (i. e. this frame is the last to play),
                    // it overrides firstSkipSample if it was set.
                    if (tocIndex == lastFrameToPlay)
                        samplesToSkip = -lastSampleToPlay;
                    ...
                    if (frame != null)
                    {
                        ...
                        if (startupFrames == 0 && firstSkipSamples > 0)
                        {
                            samplesToSkip = firstSkipSamples;
                            firstSkipSamples = 0;
                        }

                        int decompressed = decompressor.DecompressFrame(frame, decompressBuffer, 0, samplesToSkip);
                        ...
                        if (lastFrameToPlay != -1 && tocIndex > lastFrameToPlay)
                            // Enough, the stream will stop at this frame
                            break;
        ...
        protected int firstSkipSamples;
        
        private double totalMilliseconds;
        private int samplesPerFrame;


        private uint SampleByMilliseconds(uint milliseconds)
        {
        ...
        internal void TrimToPlay(uint startMs, uint finishMs)
        {
            ...
            {
                uint lastSample = SampleByMilliseconds(finishMs + 1) - 1;
                lastFrameToPlay = (int)(lastSample / samplesPerFrame);
                lastSampleToPlay = (int)(lastSample - lastFrameToPlay * samplesPerFrame);
            }

            startupFrames = 0;

            while (startupFrames < 3 && firstFrame > 0)
            {
                firstFrame--;
                startupFrames++;
            }
            ...
            // Move to first frame
            tocIndex = firstFrame;
            mp3Stream.Position = tableOfContents[firstFrame].FilePosition;

            repositionedFlag = true;

            decompressBufferOffset = 0;
            decompressLeftovers = 0;
            decompressor.ResetZeroFrames();
        ...
        public uint CurrentTimeMs
        {
            get
            {
                return (uint)(totalMilliseconds * tocIndex * samplesPerFrame / totalSamples);
            }
        }
    }
}
